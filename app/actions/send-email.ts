...
// Security constants and helper functions
const MAX_LENGTHS = {
  name: 100,
  email: 254,
  phone: 20,
...
function validateInput(input: string, maxLength: number): boolean {
  return input.length <= maxLength && /^[\p{L}\p{N}\s\-_.,@()]+$/u.test(input);
}

// Rate limiting using memory (replace with proper storage in production)
const requestLog = new Map<string, { count: number; timestamp: number }>();

function checkRateLimit(email: string): boolean {
  const now = Date.now();
  const userLog = requestLog.get(email);
  
  if (!userLog || (now - userLog.timestamp) > 3600000) {
    requestLog.set(email, { count: 1, timestamp: now });
    return true;
  }
  
  if (userLog.count >= 5) return false;
  
  userLog.count += 1;
  requestLog.set(email, userLog);
  return true;
}
...
export async function sendContactEmail(prevState: any, formData: FormData) {
  ...
  try {
    ...
    const firstName = sanitizeInput(formData.get("firstName")?.toString()?.trim() || "");
    const lastName = sanitizeInput(formData.get("lastName")?.toString()?.trim() || "");
    const email = (formData.get("email")?.toString()?.trim() || "").toLowerCase();
    const phone = sanitizeInput(formData.get("phone")?.toString()?.trim() || "");
    const address = sanitizeInput(formData.get("address")?.toString()?.trim() || "");
    const propertyType = sanitizeInput(formData.get("propertyType")?.toString()?.trim() || "");
    const description = sanitizeInput(formData.get("description")?.toString()?.trim() || "");

    // Validate input lengths and characters
    if (!validateInput(firstName, MAX_LENGTHS.name) || 
        !validateInput(lastName, MAX_LENGTHS.name) ||
        !validateInput(phone, MAX_LENGTHS.phone) ||
        (address && !validateInput(address, MAX_LENGTHS.address)) ||
        (propertyType && !validateInput(propertyType, MAX_LENGTHS.propertyType)) ||
        (description && !validateInput(description, MAX_LENGTHS.description))) {
      return {
        success: false,
        message: "Ogiltig inmatning. Kontrollera längden och använd endast tillåtna tecken.",
      };
    }
